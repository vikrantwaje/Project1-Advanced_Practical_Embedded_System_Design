...
void built_in_self_test(){
	...
	if(data == NULL || data_16 == NULL){
		...
		if(return_data !=80.0){
			printf("\n\rTemperature sensor initialisation failed.Incorrect Thigh value");
		}	
		return_data = get_Tlow(REQUEST_CELSIUS);
		if(return_data !=75.0){
			printf("\n\rTemperature sensor initialisation failed.Incorrect Tlow value");
		}

		ret_status = read_temp_fault(data);
		if(((*(data) & 0x03) !=0x00) && ret_status!=READ_REG_SUCCESS){
		printf("\n\rTemperature sensor initialisation failed. Incorrect fault bit values data = %x",*data);
		}
		
		ret_status = configure_temp_EMmode(EM_ON);
